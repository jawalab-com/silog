...
class RfqController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $rfqs = Rfq::whereIn('status', array_column(RfqStatus::cases(), 'value'))
            ...
            ->where(function ($query) use ($rfqPaid, $rfqStatus, $role) {
                ...
                } elseif ($rfqStatus === 'permintaan-perubahan') {
                    $query->where('status', '=', 'permintaan-perubahan');
                } elseif ($rfqStatus === 'sedang-dalam-pengiriman') {
                    $query->where('status', '=', 'sedang-dalam-pengiriman');
                }
            })
    ...
    public function show(Rfq $rfq, Request $request): Response
    {
        ...
        foreach ($rfqDetails as $key => $detail) {
            ...
            $data['products'][$key]['tags'] = Tag::where('slug', $detail->product->tag)->first()->toArray();
            $supplierProducts[]  = $detail->rfqSupplierProducts()->with(['detail', 'supplier', 'product', 'unit'])->get();
        }
    ...
    public function update(UpdateRfqRequest $request, Rfq $rfq)
    {
        ...
        try {
            if ($verified == 0 && $data['status'] != RfqStatus::PERMINTAAN_PERUBAHAN->value) {
                $data['status']     = RfqStatus::DITOLAK->value;
            } else if ($verified == 0 && $data['status'] == RfqStatus::PERMINTAAN_PERUBAHAN->value) {
                $data['status'] = RfqStatus::PENDING->value;
                $data['verified_1'] = 1;
                $data['verified_2'] = 1;
            }
    ...
    public function permintaanPerubahan(Request $request, Rfq $rfq)
    {
        $rfq->update([
            'status' => RfqStatus::PERMINTAAN_PERUBAHAN->value,
            'verified_1' => null,
            'verified_2' => null,
            'verified_3' => null,
            'verified_4' => null,
        ...
        RfqHistory::create([
            ...
            'user_id' => auth()->id(),
            'status' => RfqStatus::PERMINTAAN_PERUBAHAN->value,
            'description' => "Permintaan Perubahan dari pengajuan barang dengan nomor pengajuan $rfq->rfq_number dan akan dikembalikan ke pengaju",
        ]);

        return redirect()->route('rfqs.index')
            ->with('success', 'Permintaan perubahan berhasil');
    }
}
