...
const actionLabel = computed(() => {
    ...
    if (
        role === "keuangan" &&
        ["diproses", "selesai"].includes(props.rfq.status)
    ) {
        return "Upload Nota";
    }

    if (role === "admin-gudang" && props.rfq.verified_1) {
        return allAvailable || props.rfq.status == "diproses"
            ? "Kirim ke pengaju"
            : "Kirim ke purchasing";
    }
    ...
    try {
        ...
        if (confirm("Apakah Anda yakin?")) {
            form.post(route("rfqs.update", props.rfq.id), {
                ...
                /*************  ✨ Codeium Command ⭐  *************/
                /**
                 * Callback function that will be called when the request fails.
                 *
                 * The function takes an object parameter, which contains the error
                 * information.
                 *
                 * @param {Object} errors - The error object containing the errors.
                 */
                /******  5904f7d9-5b3c-44f7-9294-f0cb014c7e35  *******/
                onError: (errors) => {
...
const permintaanPerubahan = () => {
    try {
        if (confirm("Apakah Anda yakin?")) {
            form._method = "post";
            form.post(route("rfqs.permintaan-perubahan", props.rfq.id), {
                onSuccess: () => {
                    console.log("Purchase order updated successfully");
                },
                onError: (errors) => {
                ...
                <div class="flex justify-between mt-2">
                    ...
                    >
                        ...
                        >
                            ...
                            <Button
                                color="yellow"
                                @click="permintaanPerubahan"
                                type="button"
                                v-if="
                                    ['purchasing'].includes(role) &&
                                    form.status !==
                                        rfqStatus['PERMINTAAN_PERUBAHAN']
                                "
                                class="ml-2"
                            >
